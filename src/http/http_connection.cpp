...
HttpConnection::~HttpConnection() {
    MS_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            MS_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
			// 有数据了
    ...
    if(!body.empty()) {
        ...
        MS_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
            << " size=" << body.size();
		// TODO
#if 0
...
HttpResult::ptr HttpConnection::DoRequest(HttpMethod method
    ...
    req->setPath(uri->getPath()); // 这里需要考虑 path 是 空的情况
    ...
    for(auto& i : headers) {
        if(strcasecmp(i.first.c_str(), "connection") == 0) {
            if(strcasecmp(i.second.c_str(), "keep-alive") == 0) {
                req->setClose(false);  // 默认是 true， 如果是长连接，就设置成 false 
            }
            continue;
        }
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(rt == 0) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::SEND_CLOSE_BY_PEER
                , nullptr, "send request: peer closed. : " + addr->toString());
    }
    if(rt < 0) {
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        MS_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if(!ptr) {
        ...
        if(!addr) {
            MS_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
		// TODO
        ...
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            MS_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            MS_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= MNSER::GetCurrentMS())
    ...
    pool->m_conns.push_back(ptr);  // 连接释放之后，再加入到池子中
}
